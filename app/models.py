...
class User(db.Model):  # One user have many address
    __tablename__ = 'users'
    # Ensures table modifications apply
    __table_args__ = {'extend_existing': True}

    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False)
    createdAt = db.Column(
        db.DateTime, default=datetime.utcnow, name="createdAt")
    updatedAt = db.Column(db.DateTime, default=datetime.utcnow,
                          onupdate=datetime.utcnow, name="updatedAt")
...
class Order(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    payment_type_id = db.Column(db.Integer, db.ForeignKey(
        'payment_types.id'), nullable=False)
    amount = db.Column(db.Numeric(10, 2), nullable=False)

    # Relationship
    delivery_address = db.relationship(
        'OrderDeliveryAddress', backref='order', uselist=False)
    billing_address = db.relationship(
        'OrderBillingAddress', backref='order', uselist=False)
